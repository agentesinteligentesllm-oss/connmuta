...
*Size reconciliation.* Candidate (`3f289c2`): **540 authored lines**. Round-1 tip (Judgment Day round 1
corrections): **613 authored lines**. Round-2 tip (scoped re-judgment corrections, both judges
independently found `validate()`'s rewrite still leaked a match's position through total running time):
`git diff --numstat main -- src test`: `handshake.ts` 157, `sessions.ts` 115, `constants.ts` +12
(`MAX_ACTIVE_SESSIONS`, disclosed), `ipc-contract.ts` +16, `handshake.test.ts` 200, `sessions.test.ts` 119
— **619 authored lines** against the ≈370 estimate and the 400-line budget. Disclosed PR-scoped
`size:exception` (219 lines over at the round-2 tip), consistent with every slice since PR-26; no
...
8. **Judgment Day round 1** (`bus-v2-f1-pr-30-audit-001`) added `MAX_ACTIVE_SESSIONS` to
   ...
   `SessionStore.validate` to a constant-time linear scan (Judge B alone). Its scoped re-judgment found
   that rewrite still leaked a match's position through total running time (both judges independently,
   round 2) — corrected to an unconditional full scan, disclosed as enforced by review rather than a
   regression test (a functional test cannot distinguish the two by design). Full detail, both judges'
   and the independent verifier's findings, and every correction in `apply-progress.md` §PR-30.
