...
**Status (2026-09-26, session 40): F0 closed; F1 (`f1-daemon-registry-thin-client`) is 100% complete on every actual deliverable — 218 of 219 tasks, all 44 GitHub PRs (through PR #47) merged, every unit (1 through 13) closed. The one remaining item, an `sdd-init` re-run flipping `openspec/config.yaml`'s `strict_tdd` to `true`, is deliberately left for the next session at this phase boundary (DN-04). Next: the Director decides whether to archive `f1-daemon-registry-thin-client` or begin F2's own SDD cycle — see `docs/08-sessions/HANDOFF.md` for the full state and `AGENTS.md`'s own Status line for the per-PR history.**
...
| 10 | **Audit follow-ups of PR-12…PR-39 (sessions 22–39)** — filed in the backlog, none resolved silently. PR-32's own audit (session 32), PR-36's own audit (session 36) and PR-38's own audit (session 38, closing unit 11 `v1-migration`, both Judgment Day re-judgment rounds used) each filed no new row. **B-36**: PR-12's native-review advisories. **B-37**, **B-38**: cursor advance and the ledger's peer-body columns carry no token guard; no receive-side scan exists in F1. **B-39**: two wall-clock tests flake in CI. **B-40**, **B-41**: design §8.1's `poller_conflict`/`poller_rate_limited` and design §6's `secret_store_fallback` conditions have no contract. **B-42**: a SEAM header's `v1 body sha256` is not machine-checked (the v1 checkout is not in CI; pins are reproduced at audit time). **B-43**: `serve/thread.ts` still names v1's `agentbus_fetch` in a runtime message. **B-44**: a room-guard refusal inside a misbuilt transport degrades the send instead of `WRONG_ROOM` (production cannot build that state). **B-45**: the poller's 429 handling writes `TELEGRAM_RATE_LIMITED` against DATA-MODEL's `RATE_LIMITED`, audits no 429, and can clear a send-side backoff early. **B-46**: design §9 names the rate refusal `TELEGRAM_RATE_LIMITED`; the shipped tool code is `RATE_LIMITED`. **B-47**: THREAT-MODEL traces the IPC `Host` check (and no PT pins the body cap) only to the F3 panel, not to the client routes PR-29 defends. **B-48**: design §15's PT-24/PT-26 file pins (`daemon/ipc/server`, `client/handshake`) look stale against the later `ipc/{handshake,sessions,routes}` split PR-30 introduced. **B-49**: spec.md's "Binding never changes mid-session" scenario reads as freezing the whole binding, while the shipped freeze (and design's own Freeze row) is scoped to three identity fields (`bot_id`, `group_id`, `agent_id`) only. **B-50**: `daemon/ipc/routes.ts`'s `dispatchTool` forwards a caught tool-level error's message to the external caller with no defense-in-depth redaction pass, unlike every other egress site design's Redaction table names — no concrete leak demonstrated, matches the existing B-37/B-38 precedent. **B-51**: `client/handshake.ts` reuses the 70s long-poll `IPC_REQUEST_TIMEOUT_MS` for its own `GET /identity`/`POST /session` calls, risking a ~140s worst-case hang on a stalled (not refused) daemon connection before reporting `DAEMON_DOWN` — a latency concern, not a correctness bug (the timeout mechanism itself works correctly). **B-52**: `shared/ipc-contract.ts`'s `toolSuccessSchema` (a bare "any JSON object" check) and `ipcErrorSchema` are not cross-validated against each other where `client/ipc-stub.ts` classifies a response by HTTP status alone — not currently exploitable (no reachable daemon code path produces the ambiguous combination today), a forward-compatibility defense-in-depth gap. **B-53**: `client/main.ts`'s `runMcpClient` cannot supply the real MCP host-application label (`client_cursors.host`) because `IpcSession` must be constructed before `server.connect()`'s `initialize` exchange reveals it — a disclosed fixed placeholder ships instead, also reducing per-machine session distinguishability until a dedicated slice restructures `ipc-stub.ts`/`server.ts` to defer identity binding. **B-54**: `daemon/bootstrap.ts`'s `openLedger(...)` call site has the same unchecked-quarantine-status gap Judgment Day found and fixed in `migration/main.ts` — Judge A found and disclosed the shared pattern in already-merged code, Judge B rated the migration-specific instance CRITICAL; fixed there, filed here rather than touch an already-merged file from PR-37. **B-55** (not a Judgment Day finding — the first PR-level row filed under a real Arena/Alpha audit, PR-39): `design.md`:519 cites a stale, pre-`bus-v2-f1-tasks-001` v1 range/verdict (`44-101`/`AS-IS`) for `test/security/predicates.ts`; the ratified ruling and the shipped code both correctly use `25-101`/`SEAM` — text-only fix at the next `design.md` touch. | Each row carries its own dispositions in the checklist | B-36…B-55 | Nothing in F1 blocks on them; B-45 is the one with runtime effect (a send-side backoff can be forgotten after a successful poll) |
