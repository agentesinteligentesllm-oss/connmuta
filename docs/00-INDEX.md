...
| 8 | **Two file-integrity items surfaced by the PR-07a audit** (both outside that slice's frozen range, neither resolved silently): (a) `src/shared/constants.ts:3` pins a **blind-stripped** hash (`4ce5e514…`; rule-conformant `039d53a2…`) that no gate can catch, because a SEAM pin is only checked for *inequality*; (b) design §12's reuse table marks two v1 ranges **AS-IS** → `shared/tool-schemas.ts`, a module that is SEAM by construction (`bus-v2-f1-tasks-001` items 1–2) | Reported by the PR-07a Judgment Day audit, never debated (`bus-v2-f1-pr-07a-audit-001`) | B-? (recommended: one row each, or one shared row) | Nothing in F1 today; the constants re-pin invalidates PR-04's wrong-value-control table, so it needs its own audited change |
