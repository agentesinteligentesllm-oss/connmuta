...
| PT-06 | `conmuta.json` schema is strict: unknown keys, path-like strings and non-numeric ids are rejected. | shared unit | none — new (n4) | F1 |
...
| PT-14 | Every surfaced body carries `project_id`, `agent_id` and numeric `user_id` taken from the binding and the verified sender, never from the envelope's own claim. | daemon unit · `test/shared/fence.test.ts` | none — new | F1 |
