...
| PT-10 | Simulated crash between inbox insert and offset advance → the update is replayed once, never lost; the confirmed offset never exceeds the last persisted `update_id`. | daemon unit · `test/ledger/transaction.test.ts` · `test/ledger/schema.test.ts` (ledger half only: the rollback boundary that leaves a batch with nothing behind, and the `UNIQUE (bot_id, update_id)` dedup a redelivered update hits; the crash-replay scenario itself, across the inbox insert and the offset advance, stays with `ledger/inbox` — PR-12, design.md:551) | at-least-once replay, v1 tasks.md:62 | F1 |
| PT-11 | Two clients on one binding each receive the same batch; advancing one cursor does not move the other. | daemon unit | reverses v1 CHANNEL-setup.md:114-120 | F1 |
