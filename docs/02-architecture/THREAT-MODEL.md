...
| PT-07 | The thin-client bundle contains no reference to the keychain module, the fallback token path, or `api.telegram.org`; the installer's MCP entry passes no env at all. | static (client bundle) · `test/client/errors.test.ts` (the narrower "no Telegram-classification import" clause only, scoped to `client/errors.ts`'s own compiled output; the full bundle scan across keychain/token-path/`api.telegram.org` literals is `test/security/client-bundle.test.ts`, PR-40b) | v1:test/security.test.ts:176-196 pattern | F1 |
...
| PT-27 | **Static assertions, client bundle:** the full v1 set (§5.6) verbatim, plus PT-07 — subject to the ADR-0029 lazy-spawn tension: the F1 spec either scopes the `child_process` assertion so the only permitted spawn target is the daemon entry point (no shell, no user-controlled argv) or moves the spawn into a separate launcher bundle; until then the `child_process` row is a requirement, not a settled assertion. | static (client bundle) · `test/client/spawn.test.ts`, `test/client/run-state.test.ts` (argv/election unit coverage only — the "Argv never carries caller input" scenario; the multi-clause static bundle scan this row's `child_process` clause describes is `test/security/client-bundle.test.ts`, PR-40b — the `spawnImpl(process.execPath, [DAEMON_ENTRY], SPAWN_OPTIONS)` shape resolves the ADR-0029 lazy-spawn tension this row's own text names: one call site, frozen literal argv, `shell: false`) | v1:test/security.test.ts:176-273 | F1 |
| PT-28 | **Static assertions, daemon bundle:** no `child_process`; `node:fs` only in the allow-listed home-scoped modules; no `.claude/settings` or `permissions.allow`; no `deleteMessage`; `sendMessage` call sites only in transport modules, indented, and reachable only from an IPC request handler; the unbounded loop only in the poller module; the poller module imports no transport module. | static (daemon bundle) · `test/security/daemon-bundle.test.ts` (PR-40b, run against the real composition-root closure PR-40a wired — 61 files, not the pre-PR-40a 4; the shared `hasUnboundedLoopReference` predicate does not match `poller.ts`'s real `while (!signal.aborted)` shape, so this row's "unbounded loop" clause is pinned by a dedicated check against that exact shape instead, disclosed in the test's own module doc) | v1:test/security.test.ts:176-273, re-scoped | F1 |
