...
test("SendToolError carries retry_after_s when RATE_LIMITED supplies it (change (11)), and it is absent by default", () => {
	...
	for (const code of ["WRONG_ROOM", "SECRET_PATTERN_DETECTED", "UNKNOWN_THREAD"] as const) {
		assert.equal(new SendToolError(code, "refused").retry_after_s, undefined, `${code} carries no retry_after_s`);
	}
});
