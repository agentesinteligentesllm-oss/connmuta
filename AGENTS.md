...
- Alpha audits every unit **before** merge. Each phase F1–F8 is one SDD change; PRs are <= 400 lines of authored src+test, and an over-budget PR requires a **disclosed, PR-scoped exception** (seven so far: PR-06b, PR-07a, PR-07b, PR-08a, PR-08b, PR-09a (866), PR-09b (375) — see the Status line above and `apply-progress.md`). While the Arena bridge is down, that audit is **Judgment Day**, as the Status line records.
...
PR-08, PR-09a and PR-09b audit follow-ups only the Director can disposition (B-22, B-26, B-27, B-29, B-30 and B-31; B-28 is a doctor item for F2, and B-32 is the advisory set of PR-09b's review). Full board:
